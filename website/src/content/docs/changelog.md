...
- **Git sync across machines:** the RC test
  ...
  machine, and on the same machine refused with a message naming
  the *internal* project name — "My Project", which no screen shows . Rows are bound to the importing project; the one-repository-per-local-
  ...
  fails with a clear message instead of "Invalid username or token" .
...
- **Updates download in the background:** the update dialog used
...
- **Saved responses become examples:** Save response now stores
...
- **Editing and saving:** Cmd/Ctrl+S refreshes
...
- **AI Chat:** user-defined HTTP headers are supported and
