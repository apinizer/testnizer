...
interface LicenseManifest { generatedAt: string; count: number; entries: LicenseEntry[] }
...
export default function AboutModal() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      try {
        const r = (await window.api?.diagnostics?.thirdPartyLicenses?.()) as IpcResult<LicenseManifest> | undefined
        if (cancelled) return
        if (r?.success && r.data?.entries) {
          setLicenses(r.data.entries); setLicensesError(null)
        } else {
          setLicenses([]); setLicensesError(r?.error ?? t('about.licensesError'))
        }
      } catch (e) {
        if (!cancelled) { setLicenses([]); setLicensesError((e as Error).message) }
      }
    })()
  ...
  const legalBtnStyle: React.CSSProperties = {
    borderColor: 'var(--border)', background: 'var(--white)', color: 'var(--text)', fontSize: 13,
  }
    ...
    >
      ...
      >
        ...
        <div className="flex shrink-0 items-center justify-between border-b px-5 py-4" style={{ borderColor: 'var(--border)' }}>
          <div style={{ fontWeight: 600, color: 'var(--text)', fontSize: 15 }}>{t('about.title')}</div>
          <button
        ...
        <div className="flex-1 overflow-auto">
          ...
          <div className="flex flex-col items-center gap-2 px-5 py-6">
            <img src={appIcon} alt="Testnizer" style={{ width: 64, height: 64, borderRadius: 14 }} />
            <div style={{ fontSize: 18, fontWeight: 700, color: 'var(--text)' }}>Testnizer</div>
            <div style={{ fontSize: 13, color: 'var(--muted)' }}>{t('about.version')} {version || '—'}</div>
            <button
              type="button" onClick={() => openExternal(HOMEPAGE_URL)}
          ...
          <div className="flex gap-2 border-t px-5 py-3" style={{ borderColor: 'var(--border)' }}>
            <button
              type="button" disabled title={t('about.comingSoon')}
              className="flex flex-1 cursor-pointer items-center justify-center gap-1.5 rounded-md border px-3 py-1.5 disabled:cursor-not-allowed disabled:opacity-60"
              style={legalBtnStyle}
            >
          ...
          <div className="border-t" style={{ borderColor: 'var(--border)' }}>
            <div className="flex items-center justify-between px-5 py-2.5" style={{ borderBottom: '1px solid var(--border)' }}>
              <span style={{ fontSize: 13, fontWeight: 600, color: 'var(--text)' }}>{t('about.thirdPartyLicenses')}</span>
              {licenses && (
            ...
            <div style={{ maxHeight: 280, overflowY: 'auto' }}>
              {licenses === null && (
                <div style={{ fontSize: 13, color: 'var(--muted)', padding: '14px 20px' }}>{t('about.licensesLoading')}</div>
              )}
              {licenses && licenses.length === 0 && licensesError && (
                <div style={{ fontSize: 13, color: 'var(--red, #cc2200)', padding: '14px 20px' }}>{licensesError}</div>
              )}
              {licenses && licenses.length > 0 && (
                <ul style={{ listStyle: 'none', margin: 0, padding: 0 }}>
                  {licenses.map((l) => (
                    ...
                    >
                      <span className="truncate" style={{ fontSize: 13, color: 'var(--text)', marginRight: 12 }}>
                        {l.name}<span style={{ color: 'var(--muted)' }}>@{l.version}</span>
                      </span>
                      <span style={{ fontSize: 12, color: 'var(--accent-text)', flexShrink: 0, fontFamily: 'var(--font-mono, monospace)' }}>
                        {l.license || 'UNKNOWN'}
                      </span>
                    </li>
                  ))}
