...
import {
  Check, Globe, Server, Wifi, FolderOpen, GitBranch,
  TrendingUp, Eye, EyeOff, Plus, Download, Folder,
} from 'lucide-react'
...
export default function NewProjectModal() {
  ...
  const [localFileData, setLocalFileData] = useState<{ project?: { name?: string; description?: string } } | null>(null)
  ...
  useEffect(() => {
    if (show) {
      setStep(1); setDone(false); setSource('new')
      setGitCloneUrl(''); setGitCloneUser(''); setGitCloneBranch('main')
      ...
      setGitToken(''); setShowToken(false); setIsCreating(false)
    }
  }, [show])

  useEffect(() => { return () => { setGitToken(''); setGitCloneToken('') } }, [])

  if (!show) return null

  function handleClose() {
    setGitToken(''); setGitCloneToken('')
    setShow(false)
  }
  ...
  async function handleSelectLocalFile() {
    const result = await window.api?.save?.selectFile() as {
      success: boolean; data?: { filePath: string; project: { project?: { name?: string; description?: string } } }; error?: string
    }
    ...
    if (step === 1) {
      ...
      if (source === 'local') {
        if (localFilePath) {
          ...
          const result = await window.api?.save?.selectFile() as {
            success: boolean; data?: { filePath: string; project: { project?: { name?: string } } }
          }
          if (result?.success && result.data) {
            setLocalFilePath(result.data.filePath)
    ...
    if (step === 3) {
      ...
      try {
        ...
        if (projectId) {
          ...
          setDone(true)
          setTimeout(() => { setActiveProject(projectId); setShow(false) }, 1200)
        }
      } catch { /* error */ }
      setIsCreating(false)
    }
      ...
      >
        ...
        <div style={{ padding: '20px 24px', minHeight: 320, maxHeight: '65vh', overflowY: 'auto' }}>
          ...
          {step === 1 && !done && (
            <div className="flex flex-col gap-4">
              ...
              {/* Git clone fields */}
              {source === 'git' && (
                <div className="flex flex-col gap-3 rounded-lg" style={{
                  ...
                  <div className="flex gap-2.5">
                    <div className="flex-1">
                      ...
                      <input
                        value={gitCloneUser} onChange={(e) => setGitCloneUser(e.target.value)}
                        placeholder="github-user" style={inputStyle}
                      />
                    </div>
                    <div className="flex-1">
              ...
              {source === 'local' && (
                <div className="flex flex-col items-center gap-3 rounded-lg" style={{
            ...
            <div className="flex flex-col gap-4">
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder={t('newProject.descPlaceholder')} rows={2}
                  style={{
                    ...inputStyle, resize: 'none', lineHeight: 1.5, fontFamily: 'inherit',
                  }}
                />
              </div>
              ...
              <div>
                ...
                <div className="flex gap-2">
                  <OptionCard label="HTTP / REST" sub="REST, SOAP, GraphQL, SSE" active={projType === 'http'} onClick={() => setProjType('http')} icon={<Globe size={13} />} />
                  <OptionCard label="gRPC" sub="Protocol Buffers" active={projType === 'grpc'} onClick={() => setProjType('grpc')} icon={<Server size={13} />} />
                  <OptionCard label="WebSocket" sub="Realtime" active={projType === 'websocket'} onClick={() => setProjType('websocket')} icon={<Wifi size={13} />} />
                </div>
              </div>
              ...
              <div className="flex gap-4">
                ...
                <div className="flex-1">
                  ...
                  <div className="mb-2 flex gap-2">
                    ...
                    >{t('newProject.iconEmoji')}</div>
                  </div>
                  {iconOpt === 'emoji' && (
                    <>
                      <div className="mb-1.5 flex flex-wrap gap-1">
                        {EMOJIS.map((e) => (
                          <div key={e} onClick={() => { setSelectedEmoji(e); setCustomEmoji('') }}
                            className="cursor-pointer"
                            style={{
                              width: 32, height: 32, borderRadius: 7,
                              display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 17,
                              border: `1.5px solid ${selectedEmoji === e ? 'var(--accent)' : 'var(--border)'}`,
                              background: selectedEmoji === e ? 'var(--accent-light)' : 'var(--white)',
                      ...
                      <input
                        value={customEmoji}
                        onChange={(e) => { setCustomEmoji(e.target.value); setSelectedEmoji('') }}
                ...
                <div style={{ width: 120 }}>
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {COLORS.map((col) => (
                      <div key={col} onClick={() => setSelectedColor(col)}
                        className="cursor-pointer"
                        style={{
                          width: 24, height: 24, borderRadius: 6, background: col,
          ...
          {step === 3 && !done && (
            <div className="flex flex-col gap-4">
              <div>
                ...
                <div className="flex gap-2">
                  <OptionCard label={t('newProject.modeLocal')} sub={t('newProject.modeLocalSub')} icon={<FolderOpen size={13} />} active={saveMode === 'local'} onClick={() => setSaveMode('local')} />
                  <OptionCard label={t('newProject.modeGit')} sub={t('newProject.modeGitSub')} icon={<GitBranch size={13} />} active={saveMode === 'git'} onClick={() => setSaveMode('git')} />
                  <OptionCard label={t('newProject.modeBoth')} sub={t('newProject.modeBothSub')} icon={<TrendingUp size={13} />} active={saveMode === 'both'} onClick={() => setSaveMode('both')} />
                </div>
              </div>
              ...
              {(saveMode === 'git' || saveMode === 'both') && (
                <div className="flex flex-col gap-2.5 rounded-lg" style={{
                  padding: 14, background: 'var(--surface)', border: '1px solid var(--border)',
                }}>
                  <div>
                    <Label text={t('newProject.git.repoUrl')} />
              ...
              <div className="rounded-lg" style={{
                background: 'var(--surface)', border: '1px solid var(--border)', padding: 12,
              }}>
                <Label text={t('newProject.summary')} />
                <div className="flex items-center gap-3" style={{ marginBottom: 6 }}>
                  <ProjectIcon name={projName} emoji={activeEmoji} color={selectedColor} size={32} />
                  <div>
                    <div style={{ fontWeight: 600, fontSize: 13, color: 'var(--text)' }}>
                      {projName || 'Project'}
                    </div>
          ...
          {done && (
            <div className="py-5 text-center">
              ...
              >{t('newProject.openProject')}</button>
            </div>
          )}
        </div>
        ...
        {!done && (
          ...
          >
            ...
            >
              {cloning ? t('newProject.git.cloning')
                : isCreating ? t('newProject.creating')
                : step === 3 ? `${t('newProject.create')} ✓`
                : `${t('newProject.next')} →`}
            </button>
          </div>
        )}
