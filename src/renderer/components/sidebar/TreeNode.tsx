...
const ImportIcon = <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg>
...
export default function TreeNodeComponent({
  ...
  function getMenuItems(): MenuItemDef[] {
    ...
    if (isFolder || node.type === 'module') {
      ...
      if (onAddRequest) {
        items.push({ label: t('tree.addRequest'), icon: PlusIcon, action: () => onAddRequest(node) })
      }
      if (onAddFolder) {
        items.push({ label: t('tree.addFolder'), icon: FolderPlusIcon, action: () => onAddFolder(node) })
      }
      if (onRunFolder) {
        items.push({ label: t('tree.run'), icon: PlayIcon, separator: true, action: () => onRunFolder(node) })
      }
      if (onDuplicate) {
        items.push({ label: t('tree.duplicate'), icon: CopyIcon, separator: !isTopLevel, action: () => onDuplicate(node) })
      }
      if (onExport && isTopLevel) {
        items.push({ label: t('tree.export'), icon: ExportIcon, action: () => onExport(node) })
      }
      if (onImport) {
        items.push({ label: t('tree.importFolder'), icon: ImportIcon, action: () => onImport(node) })
      }
      if (onRename && canModify) {
        items.push({ label: t('tree.rename'), icon: PencilIcon, separator: true, action: startRename })
      }
      if (onDelete && canModify) {
        items.push({ label: t('tree.delete'), icon: TrashIcon, danger: true, action: () => onDelete(node) })
      }
    } else {
      ...
      if (onDelete && canModify) {
        items.push({ label: t('tree.delete'), icon: TrashIcon, danger: true, separator: true, action: () => onDelete(node) })
      }
    }
