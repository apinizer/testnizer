...
interface TestSuiteEndpointRow {
  id: string
  suite_id: string
  endpoint_id: string
  sort_order: number
...
export function registerTestSuiteHandlers(): void {
  ...
  ipcMain.handle('testSuite:list', async (_event, projectId: string) => {
    try {
      ...
      const suites = db.prepare(
        'SELECT * FROM test_suites WHERE project_id = ? ORDER BY sort_order, created_at'
  ...
  ipcMain.handle('testSuite:get', async (_event, suiteId: string) => {
    try {
      const db = getDb()
      const suite = db.prepare('SELECT * FROM test_suites WHERE id = ?').get(suiteId) as TestSuiteRow | undefined
  ...
  // ─── Create suite ─────────────────────────────────────────
  ipcMain.handle('testSuite:create', async (_event, payload: {
    project_id: string
    name: string
    description?: string
  }) => {
  ...
  // ─── Update suite ─────────────────────────────────────────
  ipcMain.handle('testSuite:update', async (_event, id: string, payload: {
    name?: string
    description?: string
    sort_order?: number
  }) => {
    try {
      ...
      db.prepare(`
        UPDATE test_suites SET
          ...
          updated_at = ?
        WHERE id = ?
      `).run(payload.name ?? null, payload.description ?? null, payload.sort_order ?? null, now, id)
      const suite = db.prepare('SELECT * FROM test_suites WHERE id = ?').get(id) as TestSuiteRow
      return { success: true, data: suite }
    } catch (e) {
      return { success: false, error: (e as Error).message }
  ...
  ipcMain.handle('testSuite:listEndpoints', async (_event, suiteId: string) => {
    try {
      ...
      const rows = db.prepare(`
        SELECT e.id, e.name, e.method, e.path, e.protocol, e.folder_id, f.name as folder_name, tse.sort_order
      ...
      `).all(suiteId) as (EndpointRow & { folder_name: string | null; sort_order: number })[]
  ...
  ipcMain.handle('testSuite:addEndpoints', async (_event, payload: {
    suite_id: string
    endpoint_ids: string[]
  }) => {
    try {
  ...
  ipcMain.handle('testSuite:removeEndpoint', async (_event, payload: {
    suite_id: string
    endpoint_id: string
  }) => {
    try {
      const db = getDb()
      db.prepare(
        'DELETE FROM test_suite_endpoints WHERE suite_id = ? AND endpoint_id = ?'
      ).run(payload.suite_id, payload.endpoint_id)
      return { success: true, data: true }
    } catch (e) {
      return { success: false, error: (e as Error).message }
    }
