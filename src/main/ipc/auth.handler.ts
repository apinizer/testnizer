...
export function registerAuthHandlers(): void {
  ...
  ipcMain.handle('auth:hasPassword', async () => {
    try {
      const db = getDb()
      const user = db
        .prepare('SELECT id FROM users WHERE auth_provider = ? AND password_hash IS NOT NULL')
  ...
  ipcMain.handle(
    ...
    async (
      ...
      try {
        ...
        let user = db.prepare('SELECT * FROM users WHERE auth_provider = ?').get('local') as
          | UserRow
          | undefined
  ...
  ipcMain.handle(
    ...
    async (
      ...
      try {
        ...
        const user = db
          .prepare('SELECT * FROM users WHERE auth_provider = ? AND password_hash IS NOT NULL')
  ...
  ipcMain.handle('auth:getSession', async (_event, token: string) => {
    try {
      ...
      const session = db
        ...
        | { id: string; user_id: string; token: string; expires_at: number; created_at: number }
        | undefined
