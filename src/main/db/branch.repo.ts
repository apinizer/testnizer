...
export function getBranchesByProject(projectId: string): BranchRow[] {
  const db = getDb()
  return db.prepare(
    'SELECT * FROM branches WHERE project_id = ? ORDER BY is_default DESC, created_at ASC'
...
export function getDefaultBranch(projectId: string): BranchRow | undefined {
  const db = getDb()
  return db.prepare(
    'SELECT * FROM branches WHERE project_id = ? AND is_default = 1'
...
export function getSaveHistory(projectId: string, limit = 10): SaveHistoryRow[] {
  const db = getDb()
  return db.prepare(
    'SELECT * FROM save_history WHERE project_id = ? ORDER BY timestamp DESC LIMIT ?'
