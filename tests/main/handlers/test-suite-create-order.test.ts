...
const listSuites = (): Promise<ListRes> => harness.invoke('testSuite:list', projectId) as Promise<ListRes>
...
describe('testSuite:create ordering (issue #57)', () => {
  ...
  it('assigns sort_order 0 and disambiguates duplicate names', async () => {
    const a = await createSuite('Dup')
    const b = await createSuite('Dup')
    expect(a.data!.sort_order).toBe(0)
    expect(b.data!.sort_order).toBe(0)
