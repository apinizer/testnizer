...
test.describe('Tur1 — DB corruption recovery [MST-279]', () => {
  // initDatabase() now quarantines a corrupt DB file (+ WAL/SHM sidecars) with
  // a `.corrupt-<timestamp>` suffix and recreates a fresh DB, so the app opens
  // normally instead of dying windowless. This test verifies: the window
  // appears, the IPC bridge works, a `.corrupt-` backup was written, and a
  // fresh valid DB took its place.
  test('MST-279 app recovers after corrupt DB file (not a valid SQLite file)', async () => {
    ...
    try {
      ...
      // The app must recover (not hang): the IPC bridge becomes available.
      await waitForApiBridge(win2)

      // Full recovery: workspace list resolves with a structured envelope and
      // a fresh-seeded default workspace.
      ...
      // The corrupt file was backed up (not deleted) with a `.corrupt-` suffix,
      // and a fresh valid DB now lives at the original path.
      const backups = fs.readdirSync(userDataDir).filter((f) => f.includes('.corrupt-'))
