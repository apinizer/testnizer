...
uiTest.describe('Folder Settings modal', () => {
  ...
  uiTest(
    'right-click folder → Settings opens the editor and saves a bearer',
    async ({ window }) => {
      ...
      const root = window.getByTestId('tree-node').filter({ hasText: E2E_PROJECT_NAME }).first()
      ...
      // Since #68 the entry prompts: an inline editor appears and nothing is
      // created until the name is confirmed.
      const draft = window.getByTestId('new-folder-input')
      await expect(draft).toBeVisible({ timeout: 8_000 })
      await draft.fill('New Folder')
